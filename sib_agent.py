...
class TransactionsTool(BaseTool):
    ...
    def _run(self, query: str) -> str:
        try:
            query = query + "today is November 5 2023"
...
recommend_prompt = PromptTemplate(
    input_variables=["human_input", "context", 'user_data','income', 'cibil_score', 'chat_history'],
    template=recommend_template
)


recommend_chain = LLMChain(
    llm=ChatOpenAI(temperature=0.2, model = 'gpt-3.5-turbo-16k', streaming=True, max_tokens=12000,openai_api_key="" ),
...
class Product_Services_Recommendation_Tool(BaseTool):
    ...
    def _run(self, query):
        try:
            data = recommend_chain.run(human_input= query, context = vector_search(query), income = 500000,cibil_score = 700, user_data = transactions._run("last 50 transactions"))
            return data
        except:
